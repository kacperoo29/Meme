...
namespace Meme {
	...
	Camera::Camera(uint32_t windowWidth, uint32_t windowHeight, float fov)
	{
		...
		glm::mat4 viewRotate = glm::rotate(viewTranslation, glm::radians(m_Rotation.y), glm::vec3(1, 1, 0));
		m_View = glm::rotate(viewRotate, glm::radians(m_Rotation.x), glm::vec3(1, 1, 0));
